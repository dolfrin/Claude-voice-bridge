...
| `tts/` | Pluggable TTS: `openai_tts.py`, `piper_tts.py` |
...
- An **Anthropic API key** (`sk-ant-...`) — the Agent SDK requires a real API key,
  not a claude.ai login.
...
| `/engine <openai\|piper>` | Switch TTS backend live (no restart needed) |
