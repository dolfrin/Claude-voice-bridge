...
# --------------------------------------------------------------------------- #
# I3: unified-inbox emit closure + run_inbox wiring
# --------------------------------------------------------------------------- #


def _inbox_event(project="qwing", text="hi", urgent=False, kind="stop", hash="h"):
    return {"project": project, "cwd": f"/x/{project}", "text": text,
            "urgent": urgent, "kind": kind, "hash": hash, "ts": 1.0}


@pytest.mark.asyncio
async def test_inbox_emit_urgent_synthesizes_and_sends_voice():
    from voice_bridge.bridge import _make_inbox_emit

    tts = FakeTTS()
    telegram = FakeTelegram()
    sessions = FakeSessions([FakeProject("qwing", voice="echo")])
    emit = _make_inbox_emit({"backend": tts}, telegram, FakeCfg(), sessions)

    await emit(_inbox_event(text="Deploy now?", urgent=True, kind="question"))

    # Text sent as [project] text via send_question; voice synthesized + passed.
    assert telegram.questions == [("qwing", "Deploy now?")]
    assert len(tts.calls) == 1
    assert telegram.question_kwargs[0].get("voice_bytes") is not None


@pytest.mark.asyncio
async def test_inbox_emit_nonurgent_is_text_only():
    from voice_bridge.bridge import _make_inbox_emit
    ...
    await emit(_inbox_event(text="✅ baigė", urgent=False, kind="stop"))

    assert telegram.questions == [("qwing", "✅ baigė")]
    assert tts.calls == []  # non-urgent: never synthesize
    assert telegram.question_kwargs[0].get("voice_bytes") is None


@pytest.mark.asyncio
async def test_inbox_emit_urgent_uses_alert_voice_when_configured():
    from voice_bridge.bridge import _make_inbox_emit
    ...
    telegram = FakeTelegram()
    sessions = FakeSessions([FakeProject("qwing", voice="echo")])
    emit = _make_inbox_emit({"backend": tts}, telegram, AlertCfg(), sessions)

    await emit(_inbox_event(text="Prašo leidimo", urgent=True, kind="permission"))

    assert tts.calls[0][1] == "shimmer"  # alert voice used for urgent send


@pytest.mark.asyncio
async def test_inbox_emit_tts_failure_still_sends_text():
    from voice_bridge.bridge import _make_inbox_emit

    tts = FakeTTS(boom=True)
    telegram = FakeTelegram()
    sessions = FakeSessions([FakeProject("qwing", voice="echo")])
    emit = _make_inbox_emit({"backend": tts}, telegram, FakeCfg(), sessions)

    await emit(_inbox_event(text="Deploy?", urgent=True, kind="question"))

    assert telegram.questions == [("qwing", "Deploy?")]  # text still delivered
    assert telegram.question_kwargs[0].get("voice_bytes") is None


@pytest.mark.asyncio
async def test_inbox_emit_unknown_project_still_sends():
    # An IDE project need not be a bridge project; emit must still deliver it
    # (voice falls back to the global voice, no crash on project() == None).
    from voice_bridge.bridge import _make_inbox_emit

    tts = FakeTTS()
    telegram = FakeTelegram()
    sessions = FakeSessions([])  # no known projects
    emit = _make_inbox_emit({"backend": tts}, telegram, FakeCfg(), sessions)

    await emit(_inbox_event(project="strangerepo", text="✅ baigė"))

    assert telegram.questions == [("strangerepo", "✅ baigė")]


@pytest.mark.asyncio
async def test_run_until_stopped_starts_and_cancels_inbox(monkeypatch):
    """run_until_stopped spawns the inbox drainer task and cancels it on shutdown."""
    import voice_bridge.bridge as bridge_mod
    ...
    async def fake_run_inbox(spool_dir, emit, stop_event, *, sleep_fn, interval=3, seen=None):
        state["called"] = True
        try:
            await asyncio.Event().wait()  # block until cancelled
        except asyncio.CancelledError:
    ...
    monkeypatch.setattr(bridge_mod, "run_inbox", fake_run_inbox)
